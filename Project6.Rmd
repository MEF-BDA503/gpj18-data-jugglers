...
```{r}
priceDif <-data.frame(diff(as.matrix(bist30 %>% select(matches("price")))))
volumeDif <- data.frame(diff(as.matrix(bist30 %>% select(matches("Volume")))))
...
priceTotal <- rowSums(priceDif)
volumeTotal <- rowSums(volumeDif)
corrDif <- cor(priceDif,volumeDif)
corrTotal <- cor(priceTotal,volumeTotal)
...
# 7) Comparison of Company Stocks

## 7.1) Equity Yields Comparison for the Selected Period
...
[11] - Difference of Row Values
[link](https://stackoverflow.com/questions/16212097/r-how-can-i-calculate-difference-between-rows-in-a-data-frame)
