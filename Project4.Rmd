...
averageFirst <- df1 %>% summarise_at(names(df1), mean, na.rm = TRUE) %>%
  select(matches("price")) 
averageLast <- df2 %>% summarise_at(names(df2), mean, na.rm = TRUE) %>%
  select(matches("price"))

momentums <- averageLast - averageFirst
momentums
```
...
## 2) Economic Volume of Companies
...
## 3) Correlation of Economic Value and Momentums
...
The information about companies and their stocks' purchase has a correlation nearly -0.1 which so too low to guess about it. That's why, economic values of companies are not good parameter for price predictions.

# 8) Conclusion

  * Kmeans shows us which prices are have highly momentum and which prices have lower momentum. Aselsan is the outlier because of its highly increase in price to the normalized value. Then, group 2 have high momenum then 3 group. We can deduce that, group 1 and group 2 stocks would be the best options for inverstors to get high income return.
